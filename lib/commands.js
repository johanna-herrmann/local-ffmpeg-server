...
const watermark = async function ({ input, text, x, y, fontsize, fontcolor, font }) {
  const xValue = Math.abs(x ?? 0);
  const yValue = Math.abs(y ?? 0);
  const fontSizeValue = Math.abs(fontsize ?? 16);
  const fontString = font ? `fontfile='${font}':` : '';
  await ffmpeg([
    '-i',
    input,
    '-vf',
    `drawtext=${fontString}text='${text ?? 'text'}':x=${xValue}:y=${yValue}:fontsize=${fontSizeValue}:fontcolor=${fontcolor ?? 'black'}`,
