...
const scale = async function (input, width, height) {
  await ffmpeg([
    ...
    input,
    '-vf',
    `scale=${Math.abs(width)}:${Math.abs(height)}`,
...
const crop = async function (input, width, height, x, y) {
  await ffmpeg([
    ...
    input,
    '-vf',
    `crop=${Math.abs(width)}:${Math.abs(height)}:${Math.abs(x)}:${Math.abs(y)}`,
